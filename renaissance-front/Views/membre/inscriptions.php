<?php
/**
 * Vue : Espace membre — Inscriptions
 * Message d’info + grille de cartes formulaires (réinscription, préinscription, etc.).
...
<body>
  <div class="layout-membre layout-membre--sidebar-collapsed">
    ...
    <div class="main-membre">
      <header class="header-membre">
        <a href="index.php?page=accueil" class="header-logo" aria-label="Accueil La Renaissance">
      ...
      <div class="content-membre">
        <h1 style="margin: 0 0 24px; color: #000000;">Inscriptions</h1>

        <!-- Message d’information sur le traitement des inscriptions -->
        <div class="card mb-2" style="padding: 12px 20px;">
          <p style="margin: 0; font-size: 0.80rem; opacity: 0.8;">Une fois votre formulaire soumis, votre inscription sera examinée par un membre de notre équipe administrative. Nous vous remercions de votre patience.</p>
        </div>

        <!-- Grille des formulaires d’inscription disponibles -->
        <div class="formulaires-grid">
          <div class="card formulaire-card">
            ...
            <p class="formulaire-card__info">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Facere ullam corrupti .</p>
            <p class="formulaire-card__label">Horaire</p>
            <p class="formulaire-card__horaire">Dimanches · 9h–13h ou 13h–17h</p>
            <p class="formulaire-card__action"><button type="button" class="btn btn-primary btn-sm btn-inscrire-activite">S'inscrire à l'activité</button></p>
          </div>
  ...
  <div id="modal-inscription-activite-membre" class="modal" role="dialog" aria-labelledby="modal-inscription-activite-membre-title" aria-modal="true" hidden>
    <div class="modal-backdrop"></div>
    <div class="modal-content modal-content--archiver">
      <div class="modal-inscription-activite-membre-step" data-inscription-act-step="confirm">
        <h2 id="modal-inscription-activite-membre-title" class="modal-title">Demande d'inscription</h2>
        <p class="modal-text">Confirmez vous votre demande d'inscription pour l'activité</p>
        <div class="modal-actions">
          <button type="button" class="btn btn-secondary modal-inscription-activite-membre-annuler">Annuler</button>
          <button type="button" class="btn btn-primary modal-inscription-activite-membre-confirmer">Confirmer</button>
        </div>
      </div>
      <div class="modal-inscription-activite-membre-step" data-inscription-act-step="merci" hidden>
        <h2 id="modal-inscription-activite-membre-title-merci" class="modal-title modal-title--success">Merci</h2>
        <p class="modal-text modal-text--success">
          Vos demande d'inscription à été prise en compte<br>
          merci
        </p>
        <div class="modal-actions">
          <button type="button" class="btn btn-success modal-inscription-activite-membre-fermer">Fermer</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <script>
    // Sécurité UX : les clics "S'inscrire à l'activité" ouvrent la modale même si script.js
    // rencontre un souci côté navigateur (handlers inline).
    (function () {
      var modal = document.getElementById('modal-inscription-activite-membre');
      if (!modal) return;

      var stepConfirm = modal.querySelector('[data-inscription-act-step="confirm"]');
      var stepMerci = modal.querySelector('[data-inscription-act-step="merci"]');
      var fond = modal.querySelector('.modal-backdrop');
      var btnAnnuler = modal.querySelector('.modal-inscription-activite-membre-annuler');
      var btnConfirmer = modal.querySelector('.modal-inscription-activite-membre-confirmer');
      var btnFermer = modal.querySelector('.modal-inscription-activite-membre-fermer');

      function ouvrir() {
        if (stepConfirm) stepConfirm.removeAttribute('hidden');
        if (stepMerci) stepMerci.setAttribute('hidden', '');
      ...
      function afficherMerci() {
        if (stepConfirm) stepConfirm.setAttribute('hidden', '');
        if (stepMerci) stepMerci.removeAttribute('hidden');

        // Le style du message vert est géré via CSS (objectif : pas de dépendance JS/couleur inline).
      }

      document.querySelectorAll('.btn-inscrire-activite').forEach(function (btn) {
      ...
      if (btnAnnuler) btnAnnuler.addEventListener('click', fermer);
      if (fond) fond.addEventListener('click', fermer);
      if (btnConfirmer) btnConfirmer.addEventListener('click', afficherMerci);
