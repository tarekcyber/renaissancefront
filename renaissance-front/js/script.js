...
(function () {
  ...
  function initCalendrier() {
    ...
    dateVue.setDate(1);
    /**
     * Événements factices (même liste pour tous les mois) :
     * - 18/.. : activité id 2
     * - 20/.. : activité id 1
     * - 22/.. : activité id 2
     *
     * Les couleurs des pastilles sont fixes (CSS) et n'utilisent pas de logique admin.
     */
    var activitesParJour = { 18: 2, 20: 1, 22: 3 };

    function couleurFixeParActivite(id) {
      var palette = { 1: '#2563eb', 2: '#8b5cf6', 3: '#10b981', 4: '#f59e0b' };
      return palette[id] || '#8b5cf6';
    }

    // Prend les couleurs directement depuis "Prochains cours" pour garder un rendu identique.
    var couleursParActivite = {};
    document.querySelectorAll('.calendrier-mini-dot[data-calendrier-activite]').forEach(function (dot) {
      var activiteId = dot.getAttribute('data-calendrier-activite');
      if (!activiteId) return;
      var couleur = dot.style.background || window.getComputedStyle(dot).backgroundColor;
      if (couleur && couleur !== '') couleursParActivite[activiteId] = couleur;
    });

    function couleurParActiviteDepuisListe(id) {
      var key = String(id);
      return couleursParActivite[key] || couleurFixeParActivite(id);
    }
    ...
    function afficher() {
      ...
      for (i = 1; i <= nJour; i++) {
        ...
        var weekEnd = d === 0 || d === 6;
        var activiteId = activitesParJour[i];
        var hasEvent = activiteId !== undefined && activiteId !== null;
        var cls = 'calendrier-day' + (weekEnd ? ' calendrier-day--weekend' : '') + (hasEvent ? ' calendrier-day--event' : '');
        var dot = hasEvent ? '<span class="calendrier-day-dot" data-calendrier-activite="' + activiteId + '" style="display:block;width:8px;height:8px;border-radius:50%;margin-top:4px;background:' + couleurParActiviteDepuisListe(activiteId) + ';" title="Cours / activité"></span>' : '';
  ...
  /** Espace membre > Inscriptions : « S'inscrire à l'activité » → confirmation puis message de remerciement */
  function initInscriptionActiviteMembre() {
    var modale = document.getElementById('modal-inscription-activite-membre');
    if (!modale) return;
    var stepConfirm = modale.querySelector('[data-inscription-act-step="confirm"]');
    var stepMerci = modale.querySelector('[data-inscription-act-step="merci"]');
    var fond = modale.querySelector('.modal-backdrop');
    var btnAnnuler = modale.querySelector('.modal-inscription-activite-membre-annuler');
    var btnConfirmer = modale.querySelector('.modal-inscription-activite-membre-confirmer');
    var btnFermer = modale.querySelector('.modal-inscription-activite-membre-fermer');

    function resetEtapes() {
      if (stepConfirm) stepConfirm.removeAttribute('hidden');
      if (stepMerci) stepMerci.setAttribute('hidden', '');
      modale.setAttribute('aria-labelledby', 'modal-inscription-activite-membre-title');
    }
    function afficherMerci() {
      if (stepConfirm) stepConfirm.setAttribute('hidden', '');
      if (stepMerci) stepMerci.removeAttribute('hidden');
      modale.setAttribute('aria-labelledby', 'modal-inscription-activite-membre-title-merci');
    }
    function fermer() {
      fermerModale(modale, resetEtapes);
    }

    document.querySelectorAll('.btn-inscrire-activite').forEach(function (btn) {
      btn.addEventListener('click', function () {
        resetEtapes();
        ouvrirModale(modale, true);
      });
    });
  ...
  function initCreerClasse() {
    ...
    var selectActivite = modale.querySelector('#classe-activite');
    var inputCouleur = modale.querySelector('#classe-couleur');

    function chargerCouleurPourActivite() {
      if (!selectActivite || !inputCouleur) return;
      var id = selectActivite.value;
      var palette = { '1': '#2563eb', '2': '#8b5cf6', '3': '#10b981', '4': '#f59e0b' };
      var couleur = palette[String(id)] || '#8b5cf6';
      inputCouleur.value = couleur;
    }

    function ouvrir() { ouvrirModale(modale, true); }
    function fermer() { if (form) form.reset(); fermerModale(modale); }

    if (btnCreer) btnCreer.addEventListener('click', function () {
      chargerCouleurPourActivite();
      ouvrir();
    });
    ...
    // À la création : juste valider puis fermer (pas de sauvegarde)
